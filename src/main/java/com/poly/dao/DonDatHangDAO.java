...
public interface DonDatHangDAO extends JpaRepository<DonDatHang, Integer> {
    ...
    // @Query("SELECT d FROM DonDatHang d WHERE DAY(d.ngayDatHang) = :day AND MONTH(d.ngayDatHang) = :month AND YEAR(d.ngayDatHang) = :year AND d.trangThai_donDatHang = '6'")
    // List<DonDatHang> findByNgayDatHangDate(@Param("day") int day, @Param("month") int month, @Param("year") int year);

    // // thống kê tháng
    // @Query("SELECT d FROM DonDatHang d WHERE MONTH(d.ngayDatHang) = :month AND d.trangThai_donDatHang = '6'")
    // List<DonDatHang> findByNgayDatHangMonth(@Param("month") int month);

    // // Thống kê theo năm
    // @Query("SELECT d FROM DonDatHang d WHERE YEAR(d.ngayDatHang) = :year AND d.trangThai_donDatHang = '6'")
    // List<DonDatHang> findByNgayDatHangYear(int year);

    // // Thống kê doanh thu hôm nay
    // @Query("SELECT d FROM DonDatHang d WHERE CAST(d.ngayDatHang AS date)  = CURRENT_DATE AND d.trangThai_donDatHang = '6'")
    // List<DonDatHang> findAllByNgayDatHangToday();

    // // Doanh thu tháng này
    // @Query("SELECT d FROM DonDatHang d WHERE MONTH(d.ngayDatHang) = MONTH(CURRENT_DATE) AND YEAR(d.ngayDatHang) = YEAR(CURRENT_DATE) AND d.trangThai_donDatHang = '6'")
    // List<DonDatHang> findAllByNgayDatHangThisMonth();

    // // Doanh thu năm hiện tại
    // @Query("SELECT d FROM DonDatHang d WHERE YEAR(d.ngayDatHang) = YEAR(CURRENT_DATE) AND d.trangThai_donDatHang = '6'")
